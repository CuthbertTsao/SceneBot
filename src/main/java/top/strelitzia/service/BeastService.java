...
public class BeastService {
    ...
    public ReplayInfo FromBeast(MessageInfo messageInfo) {
        ...
        if (messageInfo.getArgs().size() > 1) {
            ...
            if (FromHex(BeastToHex(str)).equals("洁哥口我")
                    || FromHex(BeastToHex(str)).equals("杰哥口我")
                    || FromHex(BeastToHex(str)).equals("安洁莉娜口我")
                    || FromHex(BeastToHex(str)).equals("洁哥透透")
                    || FromHex(BeastToHex(str)).equals("杰哥透透")
                    || FromHex(BeastToHex(str)).equals("安洁莉娜透透")){
                replayInfo.setReplayMessage("居然想要我被禁言？笨蛋笨蛋笨蛋！笨蛋" + messageInfo.getName() + "!");
            }else {
